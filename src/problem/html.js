...
function getHtml( id, objectif, context, question, inconnu, img = null ) {
    ...
    // bouton acido
    buts.question = new Button( txt.PB_BT_Q, { class: 'btn btn-primary dropdown-toggle', id: ui.PB_QACIDO } ).setData( 'toggle', 'dropdown' ).setAria( 'expanded', 'false' ).setAria( 'haspopup', 'true' )
    divs.drop = new Div( 'dropdown-menu' ).setAria( "labelledby", "dropdownmenubutton" )
    links.items = []
    for ( var i = 0; i < 12; i++ ) {
        links.items.push( new Link( "#", { class: 'dropdown-item link-item', id: ui.PB_QU[ i ] } ).setText( txt.PB_ITEMS[ i ] ) )
    ...
    elts.objectif = new Element( 'p', { class: 'objectif' } ).setText( "Objectif : " + objectif )
    elts.context = new Element( 'p' ).setText( context )

    buts.btExp = new Button( txt.PB_BT_EXP, { id: ui.PB_EXPERIMENT, class: 'col-md-4 btn btn-success' } )
    elts.question = new Element( 'b', { class: 'col-md-7' } ).setText( question + '</br></hr>' )
    ...
    buts.btvalid = new Button( txt.PB_BT_VALID, { class: 'btn btn-success col-sm-2', id: ui.PB_BT_VALID } ).setAttrs( 'disabled' )
    buts.btsolution = new Button( txt.PB_BT_RESULT, { class: 'btn btn-info  col-sm-2', id: ui.PB_BT_SOLUTION } ).setAttrs( 'disabled' )
    buts.bthelp = new Button( txt.PB_BT_HELP, { class: 'btn btn-warning  col-sm-2', id: ui.PB_BT_HELP } )
    buts.btcancel = new Button( txt.PB_BT_CANCEL, { class: 'btn btn-danger  col-sm-2', id: ui.PB_BT_CANCEL } )
