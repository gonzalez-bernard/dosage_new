...
import { Element, Label, Input, P, Domus, Form, Button, Div } from "../modules/dom.js";
...
var fieldsets = {}, legends = {}, buttons = {}, inputs = {}, divs = {}, labels = {}, forms = {},    elts = {}, selects = {};

// style
const ST_INPUT = "col-md-1 form-input";
const INT_VOL = new uString("Volume dans l'intervalle <br>[" + VOL_MIN + " , " + VOL_MAX + "] mL").convertHtmlChar().getHtml()
const INT_C1 = new uString("Concentration dans l'intervalle <br>[" + CONC_MIN + "  , " + CONC_MAX + "] mol/L").convertHtmlChar().getHtml();
const INT_VOLEAU = new uString("Volume dans l'intervalle <br>[" + VOL_EMIN + " , " + VOL_EMAX + "] mL").convertHtmlChar().getHtml();
...
legends.type = new Element("legend", { cls: "type-border" }).setText(txt.ES_TYPE);
inputs.ac = new Input("radio", { id: "type_ac", name: "choice_type"}).setValue(1)
    .setAttrs("checked");
labels.ac = new Label(txt.ES_AB, { cls: "radio-inline col-md-4" }).setFor("type_ac")
    .addChild(inputs.ac)
inputs.ox = new Input("radio", { id: "type_ox", name: "choice_type" }).setValue(2);
labels.ox = new Label(txt.ES_OR, { cls: "radio-inline col-md-8" }).setFor("type_ox")
    .addChild(inputs.ox);
fieldsets.type = new Element("fieldset", { cls: "type-border col-10 col-md-10" })
    .addChild(legends.type, labels.ac, labels.ox);

// Aide
buttons.info = new Button('INFO', { id: ui.ES_BT_DSPINFO_AC, cls: 'btn btn-info' })
...
selects.ac_titre = new Element("select", { cls: "form-control", id: ui.ES_ACIDEBASE_TITRE_SELECT, })
    .setText("<option>");
...
selects.ac_titrant = new Element("select", { cls: "form-control", id: ui.ES_ACIDEBASE_TITRANT_SELECT, }).setText("<option>").setAttrs('required');
...
selects.ox = new Element("select", { cls: "form-control", id: ui.ES_AUTREDOS_SELECT })
    .setText("<option>")
    .setStyle("width:40em");
...
let oInput = {
    ...
    label: {label:txt.ES_CONC_TITRE, o:{ cls: "label col-md-9" }},
    elt1: {data:"Toggle", value:"tooltip"},
    elt2: {data:"html", value:"true"},
    feedback: {feedback:[INT_C1], o:{offset:7, cls:"col-md-5"}},
    value: 0.01
} 
// champs pour concentration
inputs.titre_conc = new Input("text", oInput)

obj.replace(oInput,{id:ui.ES_TITRANT_CONC, label:{label:txt.ES_CONC_TITRANT, o:{ cls: "label col-md-9" }}})    
...
// champs pour volume
obj.replace(oInput,{id:ui.ES_TITRE_VOL, value: 10, max: VOL_MAX, min:VOL_MIN, title: INT_VOL, pattern:"[0-9.]{1,4}", label: {label:txt.ES_VOL_TITRE, o:{cls: "label col-md-9"}}, feedback:{feedback:[INT_VOL], o:{offset:8, cls:"col-md-4"}}})

inputs.titre_vol = new Input("text", oInput)

divs.volume = new Div("row").addChild(new Div("col-6 col-md-6").addChild(inputs.titre_vol))    

obj.replace(oInput,{id:ui.ES_EAU_VOL, value: 40, max: VOL_EMAX, min:VOL_EMIN, title: INT_VOLEAU, pattern:"[0-9.]{1,2}", label: {label:txt.ES_EAU_VOL, o:{cls: "label col-md-9"}},feedback:{feedback:[INT_VOLEAU], o:{offset:8, cls:"col-md-4"}}})
...
fieldsets.supp = new Element("fieldset", {
    id: ui.ES_SUPP,
    cls: "type-border col-md-12",
    style: "display:none",
});
legends.supp = new Element("legend", { cls: "type-border" }).setText(txt.ES_ESP_SUPP);

divs.supp = new Div("form-group row", 'div_supp').setID('supp');


// espèce supplémentaire concentration
obj.replace(oInput,{id:ui.ES_SUPP_CONC, value: "", max: CONC_MAX, min:CONC_MIN, title: txt.ES_MSG_INTERVAL+txt.ES_SUPP_CONC, pattern:"[0-1]{1}[0-9.]{0,3}", label: {label:txt.ES_SUPP_CONC, o:{cls: "label col-md-9"}}, feedback:{feedback:[txt.ES_MSG_INTERVAL+txt.ES_SUPP_CONC], o:{offset:8, cls:"col-md-4"}}})

inputs.supp_conc = new Input("text", oInput)

// espèce supplémentaire volume
obj.replace(oInput,{id:ui.ES_SUPP_VOL, value: "", max: VOL_MAX, min:VOL_MIN, title: txt.ES_MSG_INTERVAL+txt.ES_SUPP_VOL, pattern:"[1-9].?[0-9]{0,1}", label: {label:txt.ES_SUPP_VOL, o:{cls: "label col-md-9"}}, feedback:{feedback:[txt.ES_MSG_INTERVAL+txt.ES_SUPP_VOL], o:{offset:8, cls:"col-md-4"}}})
...
legends.exc = new Element("legend", { cls: "type-border" }).setText(txt.ES_EXC_TITRE);
elts.exc = new P(txt.ES_EXC_INFO);

// concentration
obj.replace(oInput,{id:ui.ES_EXC_CONC, value: "", max: CONC_RMAX, min:CONC_RMIN, title: txt.ES_MSG_INTERVAL+txt.ES_SUPP_VOL, pattern:"[0-5]{1}.?[0-9]{0,3}", label: {label:new uString(txt.ES_EXC_CONC).convertExpoIndice().html, o:{ cls: "col-9 col-md-9" }}, feedback:{feedback:[txt.ES_MSG_INTERVAL+txt.ES_EXC_CONC], o:{offset:8, cls:"col-md-4"}}})

inputs.exc_conc = new Input("text", oInput)

// Volume
obj.replace(oInput,{id:ui.ES_EXC_VOL, value: "", max: CONC_RMAX, min:CONC_RMIN, title: txt.ES_MSG_INTERVAL+txt.ES_SUPP_VOL, pattern:"[1-5]{1,2}", label: {label:new uString(txt.ES_EXC_VOL).convertExpoIndice().html, o:{ cls: "col-9 col-md-9" }}, feedback:{feedback:[txt.ES_MSG_INTERVAL+txt.ES_EXC_VOL], o:{offset:8, cls:"col-md-4"}}})

inputs.exc_vol = new Input("text", { id: ui.ES_EXC_VOL, cls: ST_INPUT })

divs.exc = new Div("row").addChild(new Div("col-6 col-md-6").addChild(inputs.exc_conc),new Div("col-6 col-md-6").addChild(inputs.exc_vol))

fieldsets.exc = new Element("fieldset", {
    cls: "type-border",
    id: ui.ES_EXC,
    style: "display:none",
}).addChild(legends.exc, elts.exc, divs.exc);
...
// Bouton
buttons.valid = new Button(txt.ES_BT_VALID, { id: ui.ES_BT_VALID, cls: "btn btn-success" })
    .setAttrs("disabled")
