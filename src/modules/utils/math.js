...
function calcDistance2Pts(p1, p2, factor = 1) {
    if (!isObject(p1) || !isObject(p2)) E.debugError(E.ERROR_OBJ)
...
function calcDistanceOrtho(p1, p2, p) {
    if (!isObject(p1) || !isObject(p2) || !isObject(p)) E.debugError(E.ERROR_OBJ)
...
function getCoordsTangente(point, xlim, ylim, pente, dep = 0.1) {
    if (!isObject(point)) E.debugError(E.ERROR_OBJ)
    if (!hasKey(point, 'x') || !hasKey(point, 'y')) throw new E.NotElementException()
    if (!isArray(xlim) || !isArray(ylim)) E.debugError(E.ERROR_ARRAY)
    if (!isNumeric(pente) || !isNumeric(dep)) E.debugError(E.ERROR_NUM)
